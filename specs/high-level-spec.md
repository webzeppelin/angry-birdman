...
Managing an Angry Birds 2 clan properly can be a very time consuming task for clan administrators. The goal of Angry Birdman is to make life easier by providing an online, streamlined management system the enables clan admins to:
...
In addition to making life easier for clan administrators, the secondary goal is to make the game funner and more competitive by providing a wealth of ways to visualize and track clan and individual performance.

## 2. Glossary of Key Concepts

### Angry Birds 2

Angry Birds 2 is a popular mobile game for iOS and Android that supports many different game modes, including competitive game modes where players are grouped into teams called **Clans** that are managed by privelaged members of the team.
...
A **Clan Owner** can do everything that a **Clan Admin** can do, but can also remove users from the **Clan Admin** role and make the clan inactive in the system. Whomever registered the clan is the initial **Clan Owner**, but that person can transfer that role to another user.
...
1. The user interface are clean, modern, and responsive so that Angry Birdman can be used across different types of devices.
...
- Clan Admin makes use a clan-specific admin menu to navigate admin features
...
- Anonymous user views flockpower report and graph showing the change in **fp** and **baselineFp** over time
...
| **playerName** | String | The display name of the player which can possible be later changed by the user | Input | Yes |
| **active** | True if the player is an active member of the clan, false if not | Input | Yes |
...
| **ratio** | Positive Float | The official ratio score for the clan for this battle , calculated from **score** and **fp** | Calculated | Yes |
...
| Field Name | Field Type | Description | Source | Required |
|:-----------|:----------:|:------------|:------:|:--------:|
| **actionCode** | String (PK) | Conscise, whitespace-free code for the action | Input | Yes |
...
|:-----------|:----------:|:--------------------|
| **result** | Integer | If **score** &gt; **oponentScore** then return 1. If **score** &lt; **oponentScore** then return -1. If **score** = **oponentScore** then return 0 |
| **fp** | Positive Integer | Sum the individual FP of each player and non-player, excluding non-players in reserve |
| **ratio** | Positive Float | Divide **score** by **fpBaseline** and multiply by 10 |
| **averageRatio** | Positive Float | Divide **score** by **fp** an multiple by 10 |
...
| **nonplayingCount** | Integer &gt;= 0 | Count the number of nonplayer records created for the battle, subtracking those in reserve |
...
| **averageNonplayingFpRatio** | Sum the **nonplayingFpRatio** values for each battle in the month and divide by **battleCount** |
| **averageReserveCount** | Sum the **reserveCount** values for each battle in the month and divide by **battleCount** |
| **averageReserveFpRatio** | Positive Float | Sum the **nonplayingFpRatio** values for each battle in the month and divide by **battleCount** |

### Monthly Individual Performance Data Calculations

In addition to generating summary records of the clan's performance over a month, the system will also need to calculate momthly summary records for each player's performance over the month.
...
| **averageRatio** | Sum the player's **ratio** values for each battle played during the month and divide by **battlesPlayed** |
...
| Field Name | Field Type | Calculation Process | 
|:-----------|:----------:|:--------------------|
| **battleCount** | Positive Integer | Count the number of battle records for a given month (i.e., **battleId** starts with **yearId**) |
...
| **averageNonplayingFpRatio** | Sum the **nonplayingFpRatio** values for each battle in the year and divide by **battleCount** |
| **averageReserveCount** | Sum the **reserveCount** values for each battle in the year and divide by **battleCount** |
| **averageReserveFpRatio** | Positive Float | Sum the **nonplayingFpRatio** values for each battle in the year and divide by **battleCount** |
...
| **averageRatio** | Sum the player's **ratio** values for each battle played during the year and divide by **battlesPlayed** |
...
The goal is to eventually open this project up to the community as an opensource project, so I would like to only use popular development languages, tools, and frameworks to promote future colaboration.  Accordingly, the following will be our technology stack for this project:
...
- When the UI needs to access or store data, or execute remote procedures, it will call the second layer, which is a Node.js REST-ful API
- When the API needs to access or persist data, it uses the data layer, which is implemented with PostgresSQL
...
Keyclock will serve as the identity provider (IdP) for Angry Birdman. Keycloack is also backed by PostgresSQL.

## 10. Performance Requirements

The Angry Birdman management system needs to be reliable and performant. It needs to support scalable server deployment using containers. The database needs to support redundancy and backup in production.  At the same time, the system needs to be usable in local development environments and in non-prod stage environments.
...
- Follows generally accepted practices for opensource software development using our development stack
- Implementation should track the specifications generated during planning - meaning that we need to keep our approach in the specs in sync with the approach used in the actual code we need to change our approach
- Project code should be well structured and organized to reduce the overall size of individual source code files
- The project should be thoroughly and professionally documented with in-code comments and supplemental documentation using an approach that is comparable to other popular opensource projects
- Commits into the source control should be as small and targeted at specific feature or change
