  ...
  useEffect(() => {
    if (data.playerStats && rosterData && playerActions.length === 0) {
      const actions: PlayerAction[] = data.playerStats.map((p) => {
        ...
        // Use default HOLD if actionCode is empty or not set
        const actionCode = p.actionCode && p.actionCode.trim() !== '' ? p.actionCode : 'HOLD';
        return {
          playerId: p.playerId,
  ...
  useEffect(() => {
    if (data.nonplayerStats && rosterData && nonplayerActions.length === 0) {
      const actions: PlayerAction[] = data.nonplayerStats.map((np) => {
        ...
        // Use default if actionCode is empty or not set
        const actionCode =
          np.actionCode && np.actionCode.trim() !== '' ? np.actionCode : defaultActionCode;
        return {
