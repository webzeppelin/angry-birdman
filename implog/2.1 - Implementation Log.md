...
This log documents the completion of Step 2.1 from the Angry Birdman
implementation plan: Docker Infrastructure Setup. This step establishes the
containerized local development environment with all required services for the
Angry Birdman clan management system.
...
2. Configure PostgreSQL with persistent data volumes and initial database
   creation
...
Development-specific overrides that are automatically merged with the base
configuration:
...
**Note**: This file was initially configured with an invalid Keycloak cache
setting (`KC_CACHE_STACK: local`) which was corrected during testing. The cache
configuration has been removed to use Keycloak defaults.
...
- **Keycloak Configuration**: Admin credentials, hostname, port, realm name,
  client ID
...
Created during implementation by copying `.env.example`. Contains actual
configuration values used by Docker Compose.
...
Bash script that creates multiple PostgreSQL databases based on the
`POSTGRES_MULTIPLE_DATABASES` environment variable. This enables both the
application database (`angrybirdman`) and Keycloak database (`keycloak`) to be
created from a single PostgreSQL instance.
...
**Important Note**: This script only sets up the database environment. Table
schemas will be created later via Prisma migrations (Step 2.2).
...
- `clan-context` - Includes `clanId` user attribute in JWT tokens for
  multi-tenancy
...
Created all configuration files, scripts, and documentation in the following
order:

1. `docker-compose.yml` - Base service definitions
2. `docker-compose.override.yml` - Development overrides
3. `.env.example` - Environment variable template
4. `database/postgres/init/01-create-multiple-databases.sh` - Database creation
   script
5. `database/postgres/init/02-init-angrybirdman-db.sql` - Database
...
2. Made PostgreSQL initialization script executable:
   `chmod +x database/postgres/init/01-create-multiple-databases.sh`
...
2. **PostgreSQL Validation**:
   - Verified database creation: Both `angrybirdman` and `keycloak` databases
     exist
...
1. **PostgreSQL 15 (Alpine)**:
   ...
   - Single instance serving both application and Keycloak (development
     optimization)
...
- **Production-Ready Structure**: Configuration supports production deployment
...
- **Service Discovery**: Inter-service communication via service names (e.g.,
  `postgres:5432`)
...
- **Initialization Scripts**: PostgreSQL initialization scripts mounted
  read-only
...
**Issue**: Initial `docker-compose.override.yml` included invalid cache
configuration:
...
**Error**:
`Invalid value for option 'kc.cache-stack': local. Expected values are: tcp, udp, kubernetes, ec2, azure, google.`

**Solution**: Removed cache configuration from override file to use Keycloak
defaults. Development mode (`start-dev`) handles caching appropriately without
explicit configuration.

**Lesson**: Keycloak 23's cache configuration has specific requirements. For
development mode, explicit cache configuration is unnecessary and can cause
issues.

### Challenge 2: Multiple Database Creation

**Issue**: Need to create both application database and Keycloak database from
single PostgreSQL instance.

**Solution**: Created custom initialization script
(`01-create-multiple-databases.sh`) that:
...
**Lesson**: PostgreSQL Docker image supports multiple databases through custom
initialization scripts, but requires careful scripting.

### Challenge 3: Keycloak Admin Console in WSL2 Environment

**Issue**: When accessing Keycloak admin console from Windows browser (Microsoft
Edge) while Docker runs in WSL2, the admin console displayed a loading spinner
with the error:
...
This issue persisted even in InPrivate/Incognito mode and with third-party
cookies enabled.

**Root Cause**: WSL2 networking translation between Windows and Linux causes
issues with Keycloak's iframe-based authentication checks and cookie handling.
When accessing `localhost:8080` from Windows, the request goes through WSL2's
network translation layer, which disrupts Keycloak's same-site cookie policies
and iframe communication.

**Solution**: Updated Keycloak configuration to disable strict hostname checking
for WSL2 compatibility:

1. **Modified `docker-compose.yml`**:
   - Removed explicit `KC_HOSTNAME: localhost` setting
   - Added `KC_HOSTNAME_PORT: 8080` for proper port handling
   - Kept `KC_HOSTNAME_STRICT: false` to allow flexible hostname handling

2. **Modified `docker-compose.override.yml`**:
   - Changed command from `start-dev` to `start-dev --hostname-strict=false`
   - Added
     `KC_SPI_LOGIN_PROTOCOL_OPENID_CONNECT_SUPPRESS_LOGOUT_CONFIRMATION_SCREEN: true`
   - Kept `KC_HOSTNAME_STRICT_BACKCHANNEL: false` for flexible backchannel
     communication

3. **Key Configuration Changes**:
   - Hostname set to `<request>` (visible in logs: "Hostname: <request>, ...
     Admin: <request>")
   - This allows Keycloak to accept requests from any hostname (localhost, WSL2
     IP, etc.)

**Verification**:

- Admin console became accessible at
  `http://localhost:8080/admin/master/console/`
- Successfully logged in with admin credentials
- User was able to change admin password
- No iframe timeout errors

**Lesson**: When deploying Keycloak in WSL2 environments accessed from Windows
browsers, strict hostname validation must be disabled. The
`--hostname-strict=false` flag and setting hostname to `<request>` are essential
for cross-network-boundary authentication to work properly. This is acceptable
for development but should be re-enabled with proper hostname configuration for
...
✅ **WSL2 + Windows Browser Integration**: Admin console accessible from Windows
Edge browser via localhost:8080  
✅ **Authentication Flow**: Successfully logged into admin console and changed
password  
✅ **Cross-Network Communication**: WSL2 network translation working properly
with updated configuration

## Known Limitations

1. **Development Configuration**: Current setup uses weak passwords and HTTP
   (not HTTPS). Suitable for development only.

2. **Email Verification Disabled**: Keycloak email verification is disabled.
   Production requires SMTP configuration.

3. **Single PostgreSQL Instance**: Both application and Keycloak share a
   PostgreSQL instance. Production should consider separate instances for
   isolation.

4. **No Automatic Realm Import**: Realm import requires manual step or
   uncommenting import command. Future enhancement could automate this.

5. **No Monitoring**: No monitoring/alerting configured. Production requires
   proper observability stack.

6. **WSL2-Specific Configuration**: Keycloak hostname validation is disabled for
   WSL2 compatibility. Production deployments should enable strict hostname
   checking with proper DNS configuration.
...
With the Docker infrastructure successfully implemented and validated, the
...
Step 2.1 - Docker Infrastructure Setup has been successfully completed and
validated. All three core services (PostgreSQL, Valkey, and Keycloak) are
running, healthy, and properly configured for local development.
...
The infrastructure is now ready for Step 2.2 - Database Schema Implementation,
where Prisma will be configured and database tables will be created based on the
