...
describe('MasterBattleService', () => {
    ...
    describe('getNextBattleDate', () => {
        ...
        it('should throw error if setting not found', async () => {
            await expect(service.getNextBattleDate()).rejects.toThrow('Next battle start date not configured');
        });
    });
    describe('updateNextBattleDate', () => {
        it('should update next battle date', async () => {
            ...
            futureDate.setDate(futureDate.getDate() + 10);
            await service.updateNextBattleDate({ nextBattleStartDate: futureDate.toISOString() }, 'test-user-id');
        ...
        it('should reject past dates', async () => {
            ...
            pastDate.setDate(pastDate.getDate() - 10);
            await expect(service.updateNextBattleDate({ nextBattleStartDate: pastDate.toISOString() }, 'test-user')).rejects.toThrow('must be in the future');
        });
        it('should create setting if not exists', async () => {
            const futureDate = new Date();
            futureDate.setDate(futureDate.getDate() + 10);
            await service.updateNextBattleDate({ nextBattleStartDate: futureDate.toISOString() }, 'test-user');
            const setting = await prisma.systemSetting.findUnique({
                where: { key: 'nextBattleStartDate' },
            });
            expect(setting).toBeTruthy();
    ...
    describe('createMasterBattle', () => {
        ...
        it('should reject duplicate battle IDs', async () => {
            ...
            // Use Date constructor with explicit year/month/day to avoid timezone issues
            const startDate = new Date(2025, 11, 20); // Month is 0-indexed, so 11 = December
            await expect(service.createMasterBattle({
                startDate: startDate,
            }, 'test-user')).rejects.toThrow('already exists');
        });
...
//# sourceMappingURL=masterBattle.service.test.js.map