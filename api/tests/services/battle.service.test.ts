...
import type { BattleEntry, BattleUpdate } from '@angrybirdman/common';
...
describe('BattleService with Master Battle Integration', () => {
  ...
  const masterBattleId: string = '20241201';
  ...
  describe('createBattle', () => {
    ...
    it('should reject invalid battleId not in MasterBattle', async () => {
      ...
      await expect(battleService.createBattle(testClanId, battleData)).rejects.toThrow(
        'does not exist in the master schedule'
      );
    });
    ...
    it('should reject duplicate battleId for same clan', async () => {
      ...
      await expect(battleService.createBattle(testClanId, battleData)).rejects.toThrow(
        'has already been recorded for this clan'
      );
    });
  ...
  describe('updateBattle', () => {
    ...
    it('should throw error for non-existent battle', async () => {
      ...
      await expect(battleService.updateBattle(testClanId, '19990101', updateData)).rejects.toThrow(
        'not found'
      );
    });
  });
  ...
  describe('getBattleById', () => {
    ...
    it('should throw error for non-existent battle', async () => {
      await expect(battleService.getBattleById(testClanId, '19990101')).rejects.toThrow(
        'not found'
      );
    });
  });
