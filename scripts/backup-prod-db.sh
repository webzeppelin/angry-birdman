...
################################################################################
# backup-prod-db.sh - Backup Angry Birdman production databases
#
# This script creates backups of both the application database and Keycloak
# database from the running production deployment. Designed for cron scheduling.
#
# Usage: ./scripts/backup-prod-db.sh [options]
#
# Options:
...
if ! docker compose -f docker/docker-compose.prod.yml ps postgres 2>/dev/null | grep -q "Up"; then
...
if [ -f "docker/.env.prod" ]; then
  ...
  set +a
  log "Loaded environment variables from docker/.env.prod"
else
  log_error "Environment file not found: docker/.env.prod"
  exit 1
fi
...
APP_DB="${POSTGRES_DB:-angrybirdman}"
